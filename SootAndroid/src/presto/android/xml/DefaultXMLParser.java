...
class DefaultXMLParser extends AbstractXMLParser {
  ...
  public void feedIdIntoGeneralMap(String type, String name, Integer value, boolean isSys) {
    ...
    if (isSys) {
      if (!sysRGeneralIdMap.containsKey(type)) {
        sysRGeneralIdMap.put(type, Maps.<String, Integer>newHashMap());
      }
      if (!invSysRGeneralIdMap.containsKey(type)) {
        invSysRGeneralIdMap.put(type, Maps.<Integer, String>newHashMap());
      }
      workingMap = sysRGeneralIdMap.get(type);
      invWorkingMap = invSysRGeneralIdMap.get(type);
    } else {
      if (!rGeneralIdMap.containsKey(type)) {
        rGeneralIdMap.put(type, Maps.<String, Integer>newHashMap());
      }
      if (!invRGeneralIdMap.containsKey(type)) {
        invRGeneralIdMap.put(type, Maps.<Integer, String>newHashMap());
      }
    ...
    if (Configs.verbose) {
      if (workingMap.containsKey(name) || invWorkingMap.containsKey(value)) {
        System.out.println("[VERB] DefaultXML Parser feedIdIntoGeneralMap name value conflicts at "
                + name + ":" + value);
      }
    }
  ...
  private void readRFile() {
    ...
    rGeneralIdMap.put("id", Maps.<String, Integer>newHashMap());
    rGeneralIdMap.put("layout", Maps.<String, Integer>newHashMap());
    rGeneralIdMap.put("menu", Maps.<String, Integer>newHashMap());
    rGeneralIdMap.put("string", Maps.<String, Integer>newHashMap());
    invRGeneralIdMap.put("id", Maps.<Integer, String>newHashMap());
    invRGeneralIdMap.put("layout", Maps.<Integer, String>newHashMap());
    invRGeneralIdMap.put("menu", Maps.<Integer, String>newHashMap());
    invRGeneralIdMap.put("string", Maps.<Integer, String>newHashMap());

    sysRGeneralIdMap.put("id", Maps.<String, Integer>newHashMap());
    sysRGeneralIdMap.put("layout", Maps.<String, Integer>newHashMap());
    sysRGeneralIdMap.put("menu", Maps.<String, Integer>newHashMap());
    sysRGeneralIdMap.put("string", Maps.<String, Integer>newHashMap());
    invSysRGeneralIdMap.put("id", Maps.<Integer, String>newHashMap());
    invSysRGeneralIdMap.put("layout", Maps.<Integer, String>newHashMap());
    invSysRGeneralIdMap.put("menu", Maps.<Integer, String>newHashMap());
    invSysRGeneralIdMap.put("string", Maps.<Integer, String>newHashMap());
...
*/
  ...
  private void printMap(Map theMap, String name) {
    ...
    for (Object key : keys) {
      ...
      System.out.println(String.format("KEY Type: %s, %s VALUE Type: %s, %s",
