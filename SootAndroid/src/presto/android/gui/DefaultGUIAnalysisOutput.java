...
public class DefaultGUIAnalysisOutput implements GUIAnalysisOutput {
    ...
    Map<EventType, Set<SootMethod>> result = Maps.newHashMap();
      ...
      for (NSetListenerOpNode setListenerOpNode : regs) {
        ...
        if (handlers == null) {
          ...
          result.put(listenerInstance.getEventType(), handlers);
        }
      // Set<SootMethod> handlersToAdd = flowgraph.regToEventHandlers.get(s);
      Collection<SootMethod> handlersToAdd = listenerInstance.getAllEventHandlers();
  ...
  public Set<EventType> getSupportedEvents(NObjectNode guiObject,
    ...
    for (NSetListenerOpNode setListenerOpNode : regs) {
      result.add(setListenerOpNode.getListenerInstance().getEventType());
    }
    return result;
  }
  ...
  private Set<NSetListenerOpNode> getCallbackRegistrations(NObjectNode guiObject,
    ...
    for (NOpNode opNode : NOpNode.getNodes(NSetListenerOpNode.class)) {
      ...
      if (receiverSet != null && receiverSet.contains(guiObject)) {
        EventType eventType = setListener.getListenerInstance().getEventType();
        if (condition.apply(eventType)) {
          result.add(setListener);
        }
      }
    }
  ...
  @Override
  public Set<NSetListenerOpNode> getCallbackRegistrations(NObjectNode guiObject) {
  ...
  public Set<SootMethod> getEventHandlers(NObjectNode guiObject,
    ...
    for (NSetListenerOpNode reg : registrations) {
      ListenerInstance listenerInstance = reg.getListenerInstance();
      EventType eventType = listenerInstance.getEventType();
      if (condition.apply(eventType)) {
        eventHandlers.addAll(listenerInstance.getAllEventHandlers());
      }
    }
  ...
  public Set<NContextMenuNode> explicitlyTriggeredContextMenus(Stmt s) {
    ...
    VariableValueQueryInterface variableValues =
