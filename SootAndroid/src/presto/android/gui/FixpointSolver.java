...
public class FixpointSolver {
  ...
  private static Set<SootMethod> getViewEventHandlerMethod(Set<SootClass> classes,
                                                           String methodName,
                                                           SootClass viewSootClass) {
      ...
      for (NNode target : graphUtil.reachableNodes(layoutIdNode)) {
        ...
        if (opNode.consumesLayoutId()) {
          ...
          if (Configs.sanityCheck) {
            ...
            Logger.verb("WARNING",
                    layoutIdNode + " reaching " + opNode + " in layoutIdReachability" +
                    " method");
          }
        }
      }
      ...
      for (NNode target : reachables) {
        ...
        if ((opNode instanceof NInflate2OpNode
          ...
          if (Configs.debugCodes.contains(Debug.LISTENER_DEBUG)) {
            System.out.println("[WindowAsListener] " + windowNode + " -> "
                    + opNode);
          }
      ...
      for (NLayoutIdNode layoutIdNode : entry.getValue()) {
        if (opNode instanceof NInflate1OpNode) {
          ...
          if (windows == null || windows.isEmpty()) {
            if (Configs.verbose) {
              System.out.println("[WARNING] No window reaching " + opNode +
                      ", layoutId: " + layoutIdNode);
            }
            continue;
          }
          for (NWindowNode windowNode : windows) {
            NInflNode root = doLayoutInflate(layoutIdNode.getIdValue(),
                    windowNode, opNode.getReceiver());
            if (root == null) {
              continue;
            }
  ...
  void inflationEffectsToParameters(NOpNode opNode, NInflNode inflNode,
                                    Map<NOpNode, Set<NOpNode>> reached,
...
//        vNode = flowgraph.inflNode(v.getSootClass());
      ...
      //      System.out.println("[MenuInflate] " + vNode + "--parent-->" + parent);
  ...
  NInflNode doLayoutInflate(Integer layoutId, NNode rootparent, NVarNode inflateOpNodeReciever) {
    ...
    while (!worklist.isEmpty()) {
      ...
      if (v.getSootClass() == null) {
        Logger.verb("DEBUG", "AndroidView " + v.getText() + " " + v.getId() + " "
                    + v.getOrigin() + " " +
                "SootClass is" +
                " null");
        continue;
  ...
  private Set<SootClass> getClassesWithMethod(Set<SootClass> sootClasses, String methodName,
                                              Type... parameterTypes) {
    Set<SootClass> filteredClasses = new HashSet<SootClass>();
    for (SootClass klazz : sootClasses) {
  ...
  private Set<SootClass> getClassesInHierarchyWithMethod(SootClass activityClass,
                                                         String methodName,
                                                         Type... parameterTypes) {
  ...
  private void processInlineEventHandlers(NInflNode vNode, AndroidView androidView,
                                          NNode rootNode, NVarNode activityLocalForInflateOp) {
    ...
    if (!(rootNode instanceof NActivityNode)) {
      System.out.println("Inline event handler present in " + androidView + " for " + rootNode);
      return;
    }
    ...
    for (Map.Entry<EventType, String> eventTypeMethodName : inlineClickHandlers.entrySet()) {
      ...
      Set<SootClass> classesInHierarchyWithMethod = getClassesInHierarchyWithMethod(
              activityClass, methodName, viewSootClass.getType());
      if (classesInHierarchyWithMethod.isEmpty()) {
        System.out.println("Unable to find method " +
                methodName + " in hierarchy of Activity " + activityClass);
        if (activityClass.declaresFieldByName(methodName)) {
          System.out.println("Field found with the same name " +
                  methodName + " in Activity " + activityClass);
        }
        return;
      }
      Set<SootMethod> handlerMethods = getViewEventHandlerMethod(classesInHierarchyWithMethod,
                                                                 methodName, viewSootClass);
      RefType viewType =
              vNode.getClassType() != null ? vNode.getClassType().getType() : viewSootClass.getType();
      ListenerRegistration listenerRegistration =
              listenerSpecs.getListenerRegistration(viewType.getSootClass(), EventType.click);
      if (listenerRegistration == null)
        return;
      NOpNode opNodeReferringThisView;
      NVarNode viewLocal;
      if (androidView.getId() != null &&
              (opNodeReferringThisView = getOpNodeReferringView(androidView.getId())) != null) {
      ...
      ListenerInstance listenerInstance = new ListenerInstance(Scene.v().getSootClass(
              "android.view.View$OnClickListener"),
              activityClass,
              listenerRegistration.getHandlerPrototypes(),
              EventType.click);
      listenerInstance.recordInlineEventHandler(handlerMethods);
      NSetListenerOpNode setListenerOpNode = new NSetListenerOpNode(
              listenerInstance,
              viewLocal,
              activityLocalForInflateOp,
              null,
              false,
              false);
      flowgraph.allNNodes.add(setListenerOpNode);
      for (SootMethod clickHandler : handlerMethods) {
        Local realViewLocal = jimpleUtil.localForNthParameter(
                clickHandler, listenerRegistration.position + 1);
  ...
  private NOpNode getOpNodeReferringView(Integer viewId) {
    ...
    for (NOpNode opNode : findView1OpNodes) {
      if (opNode.getParameter().idNode != null &&
          viewId.equals(opNode.getParameter().idNode.getIdValue())) {
        return opNode;
      }
    }
    return null;
...
//                "SystemR.id.content used for non-activity object.");
  ...
  boolean processSetId(NSetIdOpNode node) {
    ...
    for (NNode receiver : receiverSet) {
      ...
      if (oldIdNode != null) {
        ...
        //        System.out.println("[WARNING] View id already set for " + node);
        continue;
      }
      changed = true;
  ...
  void propagateViewToParameter(Set<NNode> solution, NOpNode node,
                                Map<NOpNode, Set<NOpNode>> reachedMap,
      ...
      for (NOpNode call : callsWithParameter) {
        ...
        for (NNode s : solution) {
          if (isValidFlowByType(s, call, VarType.Parameter)) {
            trueSolution.add(s);
            if (Configs.debugCodes.contains(Debug.WORKLIST_DEBUG)) {
              System.out.println("  [PROP] src: " + node + ", tgt: " + call +
                      ", par: " + s);
            }
          }
        }
