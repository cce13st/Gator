...
public class ExplicitForwardEdgeBuilder implements Algorithm {
   ...
   * in this stage, no edges are added to wtg
   * */
  ...
  private void buildExplicitForwardEdges(
    ...
    for (NObjectNode window : guiHierarchy.keySet()) {
      if (window instanceof NActivityNode) {
        ...
        buildDialogOrMenuForwardEdges(newEdges, wtg, window, analyzeOutput,
            guiHierarchy, viewToHandlers);
      } else {
        ...
        for (HandlerBean bean : viewToHandlers.get(view)) {
          for (SootMethod handler : bean.getHandlers()) {
            ...
            if (window instanceof NActivityNode) {
              ...
              analyzerInput = new CFGAnalyzerInput(guiWidget, handler, Filter.openActivityDialogStmtFilter);
            }
  ...
  private void buildWTGEdges(
    ...
    WTGEdge newEdge = wtgHelper.createEdge(wtg, launcher, mainNode, NLauncherNode.LAUNCHER,
        EventType.implicit_launch_event, Sets.<SootMethod>newHashSet(), RootTag.implicit_launch,
        stackOps, callbacks);
  ...
  private Multimap<NObjectNode, HandlerBean> buildWidgetHandlers(
      Multimap<NObjectNode, NObjectNode> guiHierarchy,
    ...
    for (NObjectNode window : guiHierarchy.keySet()) {
      ...
      if (window instanceof NActivityNode) {
        ...
        if (window instanceof NContextMenuNode) {
          handlers = Lists.newArrayList(new String[] {
  ...
  private void initializeGUIHierarchy(
      final WTG wtg,
      final Multimap<NObjectNode, NObjectNode> guiHierarchy,
      final Multimap<NMenuNode, NWindowNode> menuToWindows,
      final Multimap<NWindowNode, Pair<NObjectNode, NContextMenuNode>> windowToContextMenus) {
  ...
  private void generateActivityImplicitHandlers(NActivityNode activity,
    ...
    SootClass activityClass = activity.c;
    {
      ...
      SootMethod handler = wtgHelper.getCallback(activityClass,
          MethodNames.activityOnNewIntentSubSig);
      if (handler != null) {
        // HandlerBean callback = new HandlerBean(activityNode, activityNode, EventType.implicit_on_activity_newIntent, handler);
        // viewToHandlers.put(activityNode, callback);
        addHandlerBean(viewToHandlers, activity, activity, EventType.implicit_on_activity_newIntent,
            handler, true);
      }
    }
    ...
    }
    {
      ...
      SootMethod handler = wtgHelper.getCallback(activityClass,
          MethodNames.onActivityResultSubSig);
      if (handler != null) {
        // HandlerBean callback = new HandlerBean(activityNode, activityNode, EventType.implicit_on_activity_result, handler);
        // viewToHandlers.put(activityNode, callback);
        addHandlerBean(viewToHandlers, activity, activity, EventType.implicit_on_activity_result,
            handler, true);
      }
    }
    {
      // handle the rest of explicit handlers on activity node
      List<String> keyActions = Lists.newArrayList(
          "boolean onKeyDown(int,android.view.KeyEvent)",
          "boolean onKeyUp(int,android.view.KeyEvent)");
      Set<SootMethod> keyActionHandlers = guiOutput.getActivityHandlers(
          activityClass, keyActions);
      if (keyActionHandlers == null) {
        return;
      }
      for (SootMethod handler : keyActionHandlers) {
        // HandlerBean callback = new HandlerBean(activityNode, activityNode, EventType.press_key, handler);
        // viewToHandlers.put(activityNode, callback);
  ...
  private void generateDialogImplicitLifeCycleHandlers(NDialogNode dialogNode,
    ...
    for (SootMethod handler : guiOutput.getOtherEventHandlersForDialog(dialogNode)) {
      ...
      if (subsig.equals(MethodNames.dialogOnCancelSubSig)) {
        // HandlerBean callback = new HandlerBean(dialogNode, dialogNode, EventType.dialog_cancel, handler);
        ...
        // HandlerBean callback = new HandlerBean(dialogNode, dialogNode, EventType.dialog_dismiss, handler);
        // viewToHandlers.put(dialogNode, callback);
        ...
        // HandlerBean callback = new HandlerBean(dialogNode, dialogNode, EventType.dialog_press_key, handler);
        // viewToHandlers.put(dialogNode, callback);
        ...
        // HandlerBean callback = new HandlerBean(dialogNode, dialogNode, EventType.implicit_lifecycle_event, handler);
        // viewToHandlers.put(dialogNode, callback);
        addHandlerBean(viewToHandlers, dialogNode, dialogNode, EventType.implicit_lifecycle_event, handler, true);
      } else {
  ...
  private void initWindow(
      NMenuNode menu, NWindowNode owner,
      Multimap<NMenuNode, NWindowNode> menuToWindows) {
    menuToWindows.put(menu, owner);
  }

  private void buildActivityForwardEdges(
      final Multimap<WTGEdgeSig, WTGEdge> newEdges,
      final WTG wtg,
      final NObjectNode window,
      final Map<CFGAnalyzerInput, CFGAnalyzerOutput> analyzeOutput,
      final Multimap<NObjectNode, NObjectNode> guiHierarchy,
      final Multimap<NObjectNode, HandlerBean> viewToHandlers) {
  ...
  private void buildActivityForwardEdge(
      final Multimap<WTGEdgeSig, WTGEdge> newEdges,
      final WTG wtg,
      final NObjectNode sourceWindow,
      final NObjectNode targetWindow,
      final EventType event,
      final NObjectNode guiWidget,
      final Set<SootMethod> eventHandlers) {
    ...
    try {
      ...
      //Temp work around for windows that does not have correspond WTGNode
      return;
    }
    List<StackOperation> stackOps = Lists.newArrayList();
    List<EventHandler> callbacks = Lists.newArrayList();
    if (targetWindow instanceof NActivityNode) {
      ...
      WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event,
          eventHandlers, RootTag.start_activity, stackOps, callbacks);
      addEdge(newEdges, newEdge);
    } else if (targetWindow instanceof NDialogNode) {
      /* if the target window is dialog */
      WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event,
          eventHandlers, RootTag.show_dialog, stackOps, callbacks);
      addEdge(newEdges, newEdge);
    } else if (targetWindow instanceof NContextMenuNode) {
      /* if the target window is context menu */
      /*
      WTGEdge newEdge = null;
      if (!subsig.equals(MethodNames.onCreateContextMenuSubSig)) {
        newEdge = helper.createEdge(wtg, source, target, guiWidget, event,
            eventHandler, RootTag.open_context_menu, stackOps, callbacks);
      } else {
        newEdge = helper.createEdge(wtg, source, target, guiWidget, event, null,
            RootTag.open_context_menu, stackOps, callbacks);
      }
      */
      WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget, event,
          eventHandlers, RootTag.open_context_menu, stackOps, callbacks);
      addEdge(newEdges, newEdge);
      ...
      /*
      WTGEdge newEdge = null;
      if (!subsig.equals(MethodNames.onCreateOptionsMenuSubsig)) {
        newEdge = helper.createEdge(wtg, source, target, guiWidget, event, eventHandler,
  ...
  private void buildDialogOrMenuForwardEdges(
      final Multimap<WTGEdgeSig, WTGEdge> newEdges,
      final WTG wtg,
      final NObjectNode window,
      final Map<CFGAnalyzerInput, CFGAnalyzerOutput> analyzeOutput,
    ...
    for (NObjectNode view : allViews) {
      for (HandlerBean bean : viewToHandlers.get(view)) {
        ...
        boolean avoid = true;
        for (SootMethod handler : handlers) {
          CFGAnalyzerInput input = new CFGAnalyzerInput(guiWidget, handler, Filter.openActivityDialogStmtFilter);
          CFGAnalyzerOutput targetsAndAvoid = analyzeOutput.get(input);
          Assert.assertNotNull("[Error]: cfg analyze input is not processed yet: " + input, targetsAndAvoid);
          Multimap<NObjectNode, Pair<Stmt, SootMethod>> targetWindows = targetsAndAvoid.targets;
          avoid = avoid && targetsAndAvoid.avoid;
  ...
  private void buildDialogOrMenuForwardEdge(
      final Multimap<WTGEdgeSig, WTGEdge> newEdges,
      final WTG wtg,
      final NObjectNode sourceWindow,
      final NObjectNode targetWindow,
      final HandlerBean callback) {
    ...
    if (targetWindow instanceof NActivityNode) {
      /* if the target window is activity */
      // forward edge
      WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget,
          event, handlers, RootTag.start_activity, stackOps, callbacks);
      addEdge(newEdges, newEdge);
    } else if (targetWindow instanceof NDialogNode) {
      /* if the target window is dialog */
      WTGEdge newEdge = helper.createEdge(wtg, source, target, guiWidget,
          event, handlers, RootTag.show_dialog, stackOps, callbacks);
      addEdge(newEdges, newEdge);
  ...
  private void addHandlerBean(Multimap<NObjectNode, HandlerBean> beans, NObjectNode window,
      NObjectNode widget, EventType evt, SootMethod callback, boolean isWidgetKey) {
    NObjectNode key = widget;
    if (!isWidgetKey) {
      key = window;
    }
    for (HandlerBean bean : beans.get(key)) {
      if (evt != EventType.implicit_async_event
          && bean.getEvent() != EventType.implicit_async_event
          && bean.getWindow() == window
          && bean.getGUIWidget() == widget
          && bean.getEvent() == evt) {
        bean.addHandler(callback);
        return;
      } else if (evt == EventType.implicit_async_event
          && bean.getEvent() == EventType.implicit_async_event
          && bean.getWindow() == window
          && bean.getGUIWidget() == widget
          && bean.getHandlers().size() == 1
  ...
  private void handleViewAndActivityAsyncOps(
      Multimap<NObjectNode, NObjectNode> guiHierarchy,
      Multimap<NObjectNode, HandlerBean> viewToHandlers) {
  ...
  private void handleHandlerAsyncOps(
      Multimap<NMenuNode, NWindowNode> menuToWindows,
      Multimap<NObjectNode, HandlerBean> viewToHandlers) {
    ...
    for (NNode nnode : guiOutput.getFlowgraph().allNNodes) {
      if (nnode instanceof NActivityNode) {
        ...
        for (SootMethod callback : helper.getCallbacks(
            actCls,
